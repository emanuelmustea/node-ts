...
export class RouterAdapter {
  public buildRoute = ({
    ...
    for (const propertyKey in routerInstance) {
      ...
      const metadataFilteredKeys = metadataKeys.filter(key =>
        keys.includes(key)
      );
      const routeProps = metadataFilteredKeys.reduce(
        (metadataValues, currentMetadataKey) => {
          const keyValue = Reflect.getMetadata(
            currentMetadataKey,
            routerInstance,
            propertyKey
          );
  ...
  private filterQuery = (query: any, keys: string[]) =>
    keys.reduce(
      (queryParams: object, param: string) => ({
        ...queryParams,
        [param]: query[param]
      }),
      {}
  ...
  // }
  private prepareExpressFunction = (props: any): any => (
    req: Request,
    res: Response,
    next: NextFunction
  ): void => {
  ...
  private connectRouteToExpress = ({
    ...
    expressRouter[props.method](relativePath, controller);
